...
- Contraindications v1 — domain + persistence + seed (ADR-0014,
  ...
  0 (дублікат) на другому прогоні. Application-шар і API написані
  (S07E05, ADR-0014 закрито): `ContraindicationService.check_patient()`
  читає активні Medications/Diseases пацієнта, нормалізує через
  Substance→CHEBI (S07E03) і Diseases→MONDO (S07E04) словники, шукає
  збіги. `GET /contraindications/patient/{id}`, без POST (read-only
  knowledge asset). Свідомо задокументована межа: порожній результат
  НЕ означає "протипоказань немає" — здебільшого означає, що
  препарат/діагноз не потрапив у малі стартові словники (4 речовини,
  10 хвороб). Жоден ланцюжок нормалізації досі не перевірений на
  реальному пацієнтському записі — лише на тестових фікстурах.
- ICD-11 v1 — domain + persistence + application + API + importer +
  ...
  двопрохідна побудова дерева через BlockId/Grouping1-5. Seed
  ідемпотентний за id. ПОВНЕ ДЕРЕВО (S07E06, ADR-0015 п.4 закрито):
  `chapter_no=None` вимикає фільтр ChapterNo, вантажаться усі 26+
  глав — явне рішення користувача, заміна тестової підмножини
  (розділ 1, S07E02). Функція/скрипт не перейменовані (мінімальна,
  зворотно сумісна зміна), `chapter_no="01"` і далі доступний. 26
  тестів (9 domain + 4 persistence + 13 importer: 9 попередніх + 4
  на повне дерево). Реальний повний запуск seed-скрипта підтверджено
  2026-07-09: 36044/36044 рядків прочитано, 0 unresolved parent на
  ВСЬОМУ дереві, 0 невалідних/невідомого ClassKind, ідемпотентність
  підтверджена (1055 вузлів розділу 1 з S07E02 розпізнані як
  дублікати, додано 34989 нових — 1055+34989=36044).
...
## Локальні (НЕ shared) мапінг-модулі
- app/modules/contraindications/domain/disease_mapping.py (S07E04) --
  DISEASE_TO_MONDO (11 ключів / 10 MONDO ID, топ-10 за частотою серед
  374 унікальних MONDO у 1197 записах Contraindications),
  normalize_to_mondo(). Свідомо НЕ в app/shared/ -- Contraindications
  ПЕРШИЙ споживач (не другий, як BRAND_TO_SUBSTANCE), виносити тільки
  після другого підтвердженого споживача (ADR-0012 принцип). Українські
  відповідники запропоновані і явно підтверджені користувачем, не
  прийняті мовчки. Деталі -- docs/SPRINT_7_E04_SUMMARY.md.
...
- ADR-0014 — Contraindications v1 Domain Scope (закрито S07E05: domain
  + persistence + application + API усі написані)
- ADR-0015 — ICD-11 v1 Domain Scope (завершено, п.4 закрито S07E06:
  повне дерево, усі глави)
...
- Contraindications application-шар написаний (S07E05), але ЖОДЕН
  ланцюжок нормалізації (Medications.drug_name→...→CHEBI,
  Diseases.diagnosis_name→...→MONDO) не перевірений на реальному
  пацієнтському записі — лише на тестових фікстурах. Порожній
  результат `GET /contraindications/patient/{id}` НЕ означає
  "протипоказань немає" — здебільшого означає відсутність збігу в
  малих словниках (4 речовини, 10 хвороб). Розширення словників —
  точкове, за реальною потребою
- Contraindications API-шар не має формального TestClient-тесту в
...
- Diseases.diagnosis_name → MONDO стартовий словник, ADR-0014
  disease-side (S07E04) — ЗРОБЛЕНО і ПІДТВЕРДЖЕНО користувачем у
  реальному venv 2026-07-09: pytest 233/233 (217 + 17 нових), жодної
  нової міграції (чистий Python, без ORM/БД). `DISEASE_TO_MONDO`
  (11 ключів, 10 MONDO ID) — топ-10 за реальною частотою серед 374
  унікальних MONDO у 1197 записах Contraindications (перевірено
  напряму на CSV, S07E04). Розміщено локально в
  `contraindications/domain/disease_mapping.py`, НЕ в app/shared/ —
  Contraindications перший, не другий споживач. ADR-0014 оновлено:
  обидва боки мапінгу технічно існують, але жоден не перевірений на
  реальних Disease-записах (лише фікстури) — рішення про
  application-шар не прийняте автоматично. Деталі —
  docs/SPRINT_7_E04_SUMMARY.md.
- Contraindications v1 application-шар + API, ADR-0014 закриття
  (S07E05) — ЗРОБЛЕНО (ця сесія), файли записані напряму у проєкт, ще
  НЕ підтверджено у вашому venv. Перед комітом прогнати:
  ```powershell
  cd D:\FHOS\FHOS_v0.1_MVP\backend
  python -m pytest tests/ -v
  ...
  git status
  ```
  Очікування: 8 нових тестів (tests/modules/contraindications/
  test_service.py), 241/241 (233 + 8) загалом. Жодної нової міграції.
  `ContraindicationService.check_patient()`, `GET /contraindications/
  patient/{id}` — свідомо задокументована межа: порожній результат НЕ
  означає "протипоказань немає", лише відсутність збігу в малих
  словниках. Деталі — docs/SPRINT_7_E05_SUMMARY.md.
- ICD-11 v1 повне дерево, ADR-0015 п.4 закриття (S07E06) — ЗРОБЛЕНО і
  ПІДТВЕРДЖЕНО користувачем у реальному venv 2026-07-09: pytest
  245/245 (241 + 4 нових). Реальний повний запуск seed-скрипта на
  WHO-файлі: 36044/36044 рядків прочитано, 0 пропущено (невідомий
  ClassKind/невалідні), 0 unresolved parent на ВСЬОМУ дереві (гарантія
  з розділу 1, S07E02, підтверджена на повному файлі). Ідемпотентність:
...
## AI Consilium — уточнення архітектурного принципу (S07E08, продовження)

Ключове уточнення після дискусії з власником продукту й Devil Review
(ChatGPT): різниця між AI-консиліумом у FHOS і "просто хорошим
промптом до Claude/ChatGPT" -- НЕ в тому, що консиліум "розумніший".
Різниця в ДЖЕРЕЛІ висновку:

- Консиліум-як-промпт: AI відповідає з власної навченої пам'яті
  (текстовий переказ). Помилка тут -- ПРАВДОПОДІБНА і НЕПОМІТНА:
  текст читається гладко навіть якщо факт неточний.
- Консиліум-як-Domain-Reader: AI отримує ЛИШЕ структуровані дані з
  конкретних Python-об'єктів FHOS (LaboratoryResult,
  TrendRiskAssessment, Contraindication, MedicationRecord) і формулює
  висновок ВИКЛЮЧНО на основі цих об'єктів, без доступу до власної
  "загальної медичної пам'яті". Помилка тут явна (fail loud): відсутнє
  поле, помилка валідації -- одразу видно, не ховається в гладкому
  тексті.

Архітектурний наслідок: AI Consilium НЕ може (і не повинен) бути
реалізований раніше, ніж накопичиться достатньо структурованих
Knowledge Assets і Trend-механізмів у системі (Laboratory,
Reference Range, Trend Risk, Drug Interactions, Contraindications --
уже є; Diseases Trajectory, Symptom Questionnaire -- ще ні). Це не
довільне "відкласти на потім" -- це логічна залежність: чим менше
структурованих джерел під консиліумом, тим більше він змушений
"вигадувати з голови", тим ризикованіше.

AI Consilium залишається окремою майбутньою Architect Session, не
частиною поточного Scope.
