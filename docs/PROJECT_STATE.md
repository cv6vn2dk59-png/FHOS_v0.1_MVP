...
- Diseases v1 (ADR-0013, docs/SPRINT_6_E01_SUMMARY.md, дзеркально до
  Medications v1/ADR-0009) — Disease.is_active()/duration_days() через
  resolved_date (nullable, без третього статусу), diagnosis_name
  вільний текст + опціональний icd_code, без cross-module залежностей.
  API: POST /diseases/, GET /diseases/patient/{id}.
  Alembic head: e4f6a8c1d3b5.
  НЕ підтверджено ще у реальному venv — див. "Наступна робота".
...
- Diseases v1 (ADR-0013) — ЗРОБЛЕНО (ця сесія, 2026-07-08), підтверджено
  лише в ізольованому Linux-середовищі (див. розділ нижче), НЕ у
  вашому реальному venv. Перед комітом прогнати:
  ```powershell
  ...
  150 (попередніх) + 15 нових (11 domain + 4 service, Diseases) = 165 passed.
