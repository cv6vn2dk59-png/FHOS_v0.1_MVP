...
def test_multi_ai_consilium_builds_mock_knee_orchestration_with_consistent_round_one_input():
    with make_uow() as uow:
        ...
        assert all(item["prompt_version"] == "multi-ai-consilium.v1" for item in payload["rounds"][0]["responses"])
...
def test_multi_ai_consilium_preserves_round_one_and_persists_versions():
    with make_uow() as uow:
        ...
        assert run.comparison_algorithm_version == "multi-ai-comparison.v1"
        ...
        assert all(item.prompt_version == "multi-ai-consilium.v1" for item in participants)
